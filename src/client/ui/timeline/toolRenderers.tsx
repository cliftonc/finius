...
function Mono({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <span className={clsx("font-mono text-xs text-foreground/85", className)}>{children}</span>
  );
}
...
function KV({ k, v }: { k: string; v: ReactNode }) {
  return (
    <span className="inline-flex items-baseline gap-1 min-w-0">
      <span className="text-2xs text-foreground/45">{k}:</span>
      <span className="font-mono text-xs text-foreground/85 truncate">{v}</span>
    </span>
  );
}
...
const renderers: Record<string, ToolRenderer> = {
    ...
    summary: (i) => {
      ...
      return (
        <span className="flex items-center gap-2 min-w-0 flex-1">
          ...
          {timeout != null && (
            <span className="text-2xs text-foreground/40 shrink-0">{String(timeout)}s</span>
          )}
        </span>
      );
    },
    ...
    summary: (i) => {
      ...
      return (
        <span className="flex items-center gap-3 min-w-0 flex-1">
          ...
          {offset != null && (
            <span className="text-2xs text-foreground/45 shrink-0 font-mono">
              L{String(offset)}
    ...
    summary: (i) => {
      ...
      return (
        <span className="flex items-center gap-3 min-w-0 flex-1">
          ...
          {lines > 0 && (
            <span className="text-2xs text-foreground/45 shrink-0 font-mono">{lines} lines</span>
          )}
        </span>
      );
    },
  ...
  edit_file: {
    summary: (i) => {
      ...
      return (
        <span className="flex items-center gap-3 min-w-0 flex-1">
          <Mono className="truncate flex-1">{path}</Mono>
          {replaceAll && <Badge color="warning">replace_all</Badge>}
        </span>
      );
    },
  },
    ...
    summary: (i) => {
      ...
      return (
        <span className="flex items-center gap-2 text-2xs shrink-0">
          <span className="text-foreground/60 font-mono">{todos.length} todos</span>
          {Object.entries(byStatus).map(([k, v]) => (
            <Badge
              key={k}
              color={
                k === "completed" ? "success" : k === "in_progress" ? "warning" : "default"
              }
            >
              {k.replace("_", " ")} {v}
            </Badge>
  ...
  task: {
    summary: (i) => (
      <span className="flex items-center gap-2 min-w-0 flex-1">
        <Badge color="secondary">{str(i.subagent_type) || "agent"}</Badge>
        <span className="text-xs text-foreground/80 truncate"><Trunc max={100}>{str(i.description) || str(i.prompt)}</Trunc></span>
      </span>
    ),
  },
  todowrite: {
    summary: (i) => {
      const todos = Array.isArray(i.todos) ? i.todos : [];
      return <span className="text-2xs text-foreground/60 font-mono">{todos.length} todos</span>;
    },
  },
...
function mcpSummary(_toolName: string, input: Input): JSX.Element {
  ...
  if (entries.length === 0) {
    return <span className="text-2xs text-foreground/40">no params</span>;
  }
  return (
...
export function renderToolSummary(toolName: string, input: Input): JSX.Element {
  ...
  if (entries.length === 0) {
    return <span className="text-2xs text-foreground/40">no args</span>;
  }
  return (
    <span className="flex items-center gap-3 min-w-0 flex-1">
      {entries.map(([k, v]) => (
