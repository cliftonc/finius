...
export function MessageCard({
  ...
  return (
    <div
      className={clsx(
        "border border-default-200 rounded-md overflow-hidden",
        tint === "user" ? "bg-primary/5" : "bg-content1",
        isNew && "message-new",
      )}
    >
      <div
        className={clsx(
          "flex items-center gap-2 px-3 border-b border-default-200",
          dense ? "py-1" : "py-1.5",
        )}
      >
        <div className="flex items-center gap-2 min-w-0 flex-1">{title}</div>
        {headerRight}
        <span className="text-2xs text-foreground/40 font-mono shrink-0">
